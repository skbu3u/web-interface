...
import { API_PARTS_ADD, API_EQUIP_GET } from '../../Apiconst/apiConstants';
...
export default function ModalAddParts({ open, setOpen }) {
  ...
  // копия массива лучшая версия
  const databestCopy = JSON.parse(JSON.stringify(data));
  // копия массива первая версия
  const dataCopy = { ...data };
  // добавление элемента в каждую строку массива
  const addNew = (data) => [...data].map((el) => {
    el.label = JSON.stringify(data[2].name); return el;
  });
  // делаем массив из имен
  const newdata = data.map((item) => item.name);
  // добавление в массив новый элементов
  const newnewdata = newdata.push({
    value: newdata.name,
    label: newdata.name,
  });

  // делам из элементов массива строку
  ...
  async function postdata() {
    ...
    try {
      const res = await fetch(API_PARTS_ADD, {
        method: 'post',
        headers: {
            ...
            >
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <div className="rounded-md shadow-sm -space-y-px pt-8">
                  <div>
                    <label className="text-gray-500 text-sm">Стоймость</label>
                    <input
                      id="price"
                      name="price"
                      type="text"
